...
def compute_metrics(log: Dict[str, Any]) -> Dict[str, Any]: # Compute summary metrics from a simulation log
    decisions = log.get("decisions", [])
    n = len(decisions)

    if n == 0: return {"n": 0}
    correct_total = ai_c = h_c = ai_n = h_n = defer = latency = 0 # Counters

    for d in decisions: # Iterate over decisions
        eff = d.get("effect", {})
