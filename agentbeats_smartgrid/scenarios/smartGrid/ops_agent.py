...
def choose_action(state: Dict[str, Any]) -> Dict[str, Any]:
    raw = state.get("raw", {})
    alert = raw.get("alert", False)
    load_level = raw.get("load_level", "medium")

    if alert and load_level == "high":
        return {"type": "shed_load", "reason": "high load + active alert"}
    elif alert:
        return {"type": "inspect", "reason": "alert present but load not high"}
    else:
        return {"type": "do_nothing", "reason": "no active alert"}
