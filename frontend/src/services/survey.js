...
export const fetchSurveyAggregates = async (pilotTag = null) => {
  ...
  const response = await api.get("/v1/survey/aggregate", { params });
  return response.data;
};

export async function fetchVersions(pilotTag) {
  if (!pilotTag) return [];
  const { data } = await api.get("/v1/survey/versions", {
...
export async function fetchQuestionAverages(pilotTag, appVersion) {
  ...
  const { data } = await api.get("/v1/survey/question-averages", {
