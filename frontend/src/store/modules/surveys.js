...
export default {
  ...
  getters: {
    ...
    surveyByPilotTag: (state) => (pilotTag) => {
      return state.surveyResponses.filter(
        (survey) => survey.pilot_tag === pilotTag
      );
    },
  ...
  actions: {
    ...
    async fetchVersions({ commit }, pilotTag) {
      ...
      try {
        const response = await fetch(
          `/api/v1/survey/versions?pilot_tag=${encodeURIComponent(pilotTag)}`
        );
    ...
    async fetchVersionSummary({ commit }, { pilotTag, appVersion }) {
      ...
      try {
        const response = await fetch(
          `/api/v1/survey/summary?pilot_tag=${encodeURIComponent(
            pilotTag
    ...
    async compareVersions({ commit }, { pilotTag, versionA, versionB }) {
      ...
      try {
        const response = await fetch(
          `/api/v1/survey/compare?pilot_tag=${encodeURIComponent(
            pilotTag
          )}&version_a=${encodeURIComponent(
            versionA
          )}&version_b=${encodeURIComponent(versionB)}`
        );
    ...
    async fetchQuestionAverages({ commit }, { pilotTag, appVersion }) {
      ...
      try {
        const response = await fetch(
          `/api/v1/survey/question-averages?pilot_tag=${encodeURIComponent(
            pilotTag
          )}&app_version=${encodeURIComponent(appVersion)}`
        );
