...
export function useConfigurationForm(props) {
  ...
  const formTitle = computed(() =>
    isEditMode.value ? "Edit Evaluation Configuration" : "Create Evaluation Configuration"
  )
  ...
  const isFormValid = computed(() => {
    return config.application_name.trim() &&
  ...
  const loadConfig = async () => {
    ...
    try {
      await store.dispatch('configuration/fetchConfigurationById', props.configId)
  ...
  const submitForm = async () => {
    ...
    try {
      ...
      if (isEditMode.value) {
        ...
        response = await store.dispatch('configuration/createConfiguration', config)
      }

      store.dispatch('ui/showSuccess', `Configuration ${isEditMode.value ? 'updated' : 'created'} successfully`)
      ...
      store.dispatch('ui/showError', `Failed to ${isEditMode.value ? 'update' : 'create'} configuration`)
    } finally {
